...
cd deadweight.gdt
git checkout v0.2.0
go build -ldflags "-X main.version=0.2.0" -o ./bin/deadweight.gdt ./cmd/deadweight.gdt
./bin/deadweight.gdt --version
```

Prebuilt release binaries are planned for a future distribution update; `v0.2.0` does not promise downloadable binary archives or package-manager formulas.
...
# Explain how the root reaches each scene dependency.
deadweight.gdt tree res://levels/city.tscn
...
# Compare two previously captured portable reports offline.
deadweight.gdt diff .deadweight/city-baseline.json city-candidate.json
```
...
MVP 0.2 adds a machine-readable format to report-producing commands. Text remains the default and retains its existing bytes and color behavior:
...
MVP 0.2 is additive at the CLI boundary. `inspect` and `check` still default to the byte-compatible 0.1 text presentation, built-in preset values and the eight metric definitions remain frozen, and exit codes `0`, `1`, `2`, and `3` retain their meanings. The strict project configuration remains schema version 1. Machine-readable reports also remain schema version 1; new optional evidence is covered by the committed schema and deterministic goldens.

The new `tree`, `diff`, and `profiles` commands and the `--format`, `--metric`, and `--top` selectors require explicit use. Existing scripts that do not select them keep the 0.1 behavior. See [the MVP 0.2 acceptance matrix](docs/MVP_0.2_ACCEPTANCE.md) for the cross-checkout, supported-OS, and frozen-text evidence.

## Report baselines and deterministic diffs

`diff` compares two compatible schema-v1 `inspect`, `tree`, or `check` documents without reading a Godot project or using the network. The reports must have the same kind and `res://` root scene. Tool versions, JSON whitespace, and object-field order do not count as changes.

Capture and review a baseline reproducibly:

```bash
mkdir -p .deadweight
deadweight.gdt inspect res://levels/city.tscn --project . --format json \
  > .deadweight/city-baseline.json

deadweight.gdt inspect res://levels/city.tscn --project . --format json \
  > city-candidate.json
deadweight.gdt diff .deadweight/city-baseline.json city-candidate.json
deadweight.gdt diff .deadweight/city-baseline.json city-candidate.json --format json
```

Metric changes use signed absolute deltas (`after - before`). They are labeled `regression`, `improvement`, or `uncertain` from each metric's confidence. In particular, a smaller partial value is not claimed as an improvement when the missing evidence could reverse the result. Older valid schema-v1 reports without per-metric confidence inherit their report-wide reliability conservatively.

Enforcement is disabled by default. Select only the guardrails required by CI; `--fail-on-increase` is repeatable:

```yaml
- name: Capture candidate scene report
  run: >-
    deadweight.gdt inspect res://levels/city.tscn
    --project . --format json > city-candidate.json

- name: Compare scene baseline
  run: >-
    deadweight.gdt diff .deadweight/city-baseline.json city-candidate.json
    --fail-on-increase nodes
    --fail-on-increase mesh_instances
    --fail-on-reliability
```

A selected proven increase exits `1`. A selected increase that cannot be proven from available confidence, or an enforced reliability degradation, exits `3`. Invalid, unreadable, oversized, or incompatible inputs exit `2`; a valid comparison without a triggered policy exits `0`. Text and JSON always render the complete non-fatal comparison before returning `1` or `3`.

When a reviewed change is intentional, regenerate the checked-in baseline with the same command and environment, compare it once more against the previous file, review the JSON diff, then commit the replacement baseline. Baseline paths themselves are never serialized, so reports remain portable across checkout locations and supported operating systems.

## Explainable dependency trees

Use `tree <scene>` to explain the authoritative graph already retained by recursive analysis. Text is the default; `--format json` adds a required `dependency_tree` payload to the complete version-one analysis document.
...
Rows are a deterministic depth-first preorder. Each row represents one compacted declaring edge, shows `instance` or `inheritance`, its local occurrence multiplicity, and conservative reliability. A repeated declaration therefore remains one `×N` row; the number is not multiplied into a transitive path total. A shared target in a diamond expands on its first portable path and later appears as a `back-reference`, so output remains bounded to the retained nodes and edges.

Unresolved targets remain visible with their stable classification and available resource/path context. They preserve partial lower-bound evidence; inheritance rows are approximate. Resolved cycles remain fatal `SB2002` errors rather than back-references. The command does not scan a project, invoke Godot, resolve UID-only/imported contents, merge full inherited overrides, or produce DOT/Graphviz/HTML output.

## Per-scene contributions

MVP 0.2 can append a ranked contribution view to `inspect`. Both selectors are explicit and presentation-only:
...
The repository quality gate runs build, tests, race tests, and vet on Linux, macOS, and Windows, plus golangci-lint on Linux. A separate Ubuntu job builds the standalone binary and checks the pinned official Godot demo-project corpus without installing Godot. See [`.github/workflows/ci.yml`](.github/workflows/ci.yml), the [MVP 0.2 acceptance matrix](docs/MVP_0.2_ACCEPTANCE.md), and the [current release checklist](docs/RELEASE_0.2.0_CHECKLIST.md).
...
- **0.2.1 candidate:** deeper `.tres` dependency-closure analysis;
- **0.3 hard Godot semantics:** UID resolution, full inherited-scene override/editable-child semantics, optional Godot headless/import bridging, imported scene expansion, and custom class hierarchy resolution;
- **ecosystem release:** an official GitHub Action, SARIF/JUnit/annotations and PR comments, project-wide scans, include/ignore patterns, release binaries, package-manager distribution, watch mode, incremental caches, and editor/UI integrations.
